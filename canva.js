...
module.exports = async function createImage(skins) {
    ...
    ctx.fillText("TIENDA DE OFERTAS", 1370 / 2, 100);
    ...
    ctx.fillText(`${date.getDate()} ${date.toLocaleString('default', { month: 'short' }).toUpperCase()} - ${date2.getDate()} ${date2.toLocaleString('default', { month: 'short' }).toLocaleUpperCase()}`, 1370 / 2, 150);
    ...
    ctx.fillText("GENERADO POR RUSHWTF", 1370 / 2, 1195 - 20);
    ctx.shadowColor = "#E5DCCA";
    ctx.shadowBlur = 0;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;
